...
# POLYWINS
# check if we can rename base0C in colors.ini examples
# SETTINGS {{{ ---
active_color=$(sed -n 's/base0C *= *\([^ ]*.*\)/\1/p' < "${XDG_CONFIG_HOME-${HOME}/.config}/polybar/colors.ini")

active_text_color="${active_color}"
active_bg=
active_underline="${active_color}"
